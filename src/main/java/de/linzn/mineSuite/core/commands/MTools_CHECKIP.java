...
public class MTools_CHECKIP implements ICommand {
    private MineSuiteCorePlugin plugin;
    private String permission;


    public MTools_CHECKIP(MineSuiteCorePlugin plugin, String permission) {
        this.plugin = plugin;
    ...
    public boolean runCmd(CommandSender sender, String[] args) {
        ...
        if (args.length < 3) {
            player.sendMessage(LanguageDB.COMMAND_USAGE.replace("{command}", "/mtools checkip <firstplayer> <secondplayer>"));
            return true;
        }

        String firstPlayer = args[1];
        String secondPlayer = args[2];
        JClientBungeeOutput.request_ip_compare(player.getUniqueId(), firstPlayer, secondPlayer);
